...
const MenuSection = () => {
  ...
  const currentMenu = activeCategory === "food" ? foodMenu : activeCategory === "beverage" ? beverageMenu : cakesMenu;
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="text-center mb-12 md:mb-16">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <button onClick={() => setActiveCategory("food")} className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-medium transition-all ${activeCategory === "food" ? "bg-primary text-primary-foreground shadow-lg" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>
              <UtensilsCrossed className="w-4 h-4" />
              Food
            </button>
            <button onClick={() => setActiveCategory("beverage")} className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-medium transition-all ${activeCategory === "beverage" ? "bg-primary text-primary-foreground shadow-lg" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>
              <Coffee className="w-4 h-4" />
              Drinks
            </button>
            <button onClick={() => setActiveCategory("cakes")} className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-medium transition-all ${activeCategory === "cakes" ? "bg-primary text-primary-foreground shadow-lg" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          {currentMenu.map(subcategory => <div key={subcategory.title} className="bg-secondary/30 rounded-2xl p-6 md:p-8">
              <h3 className="font-serif text-xl md:text-2xl font-semibold text-foreground mb-2">
                {subcategory.title}
              </h3>
              {subcategory.note && <p className="text-accent text-sm italic mb-4">
                  {subcategory.note}
                </p>}
                ...
                {subcategory.items.map(item => <div key={item.name} className="border-b border-border/50 pb-3 last:border-0 last:pb-0">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        {item.description && <p className="text-muted-foreground text-sm mt-1">
                            {item.description}
                          </p>}
                      </div>
                      <div className="text-right shrink-0">
                        {item.sizes ? <div className="space-y-1">
                          ...
                          </div> : item.price ? <span className="text-accent font-semibold">
                            {item.price}
                          </span> : null}
                      </div>
                    </div>
